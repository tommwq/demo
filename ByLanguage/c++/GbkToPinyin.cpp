...
std::string FindPinyinLetter(int nCode);

std::string GbkToPinyin(std::string strText)
{
    ...
    for (int i=0; i<strText.length(); i++)
    {
        ...
        std::string strRes=FindPinyinLetter(nCode);
...
int main(int argc, char **argv){
    unsigned char buffer[] = {
        0xC7, 0xE0, 0xBA, 0xA3, 0xB0, 0xE5, 0xBF, 0xE9,
    };
    std::cout << GbkToPinyin("中华人民共和国") << std::endl;
    std::string s(buffer, buffer + 8);
    std::cout << GbkToPinyin(s) << std::endl;
    return 0;
}
